...
def init(app):
    ...
    async def config_command(_, message: Message):
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("➕ Warn Limit", callback_data=f"cfg:warn_inc:{chat_id}"),
                InlineKeyboardButton("➖ Warn Limit", callback_data=f"cfg:warn_dec:{chat_id}")
            ],
            [
                InlineKeyboardButton("🔒 Mute", callback_data="cfg:punish:mute"),
                InlineKeyboardButton("🚫 Ban", callback_data="cfg:punish:ban"),
                InlineKeyboardButton("⚠️ Warn Only", callback_data="cfg:punish:warn")
            ]
        ])
    ...
    async def update_punishment_mode(client, cb: CallbackQuery):
        ...
        await cb.answer(f"✅ Punishment mode set to: {mode}", show_alert=True)
        await cb.message.delete()
        await config_command(client, cb.message)
