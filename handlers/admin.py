...
from pyrogram import filters from pyrogram.types import Message, CallbackQuery, ChatPermissions, InlineKeyboardMarkup, InlineKeyboardButton from config import OWNER_ID from utils.sudo import is_sudo from database.core import get_all_whitelist from database.config import get_config, set_warn_limit, set_punishment_mode

def init(app): @app.on_message(filters.command("ping")) async def ping(_, message: Message): if not is_sudo(message.from_user.id): return await message.reply("🚫 You are not authorized to use this.") await message.reply("🏓 <b>Pong!</b> Bot is alive and responding.")
