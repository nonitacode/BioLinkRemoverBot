...
from pyrogram import filters from pyrogram.types import Message, ChatPermissions, InlineKeyboardMarkup, InlineKeyboardButton from utils.filters import contains_link from database.core import is_whitelisted, increment_violations from database.config import get_config from config import LOG_CHANNEL

def init(app): @app.on_message(filters.group & filters.text) async def check_user_identity(_, message: Message): user = message.from_user chat_id = message.chat.id
...
if not user or user.is_bot:
    ...
    try:
        ...
        if member.status in ("administrator", "creator"):
            # Optional logging for skipped admin scans
            await app.send_message(LOG_CHANNEL, f"✅ Skipped scan for admin: <a href='tg://user?id={user_id}'>{user.first_name}</a> [{user_id}] in <code>{chat_id}</code>")
            return
    except:
        pass
