...
from database.allow_db import allow_user
...
async def allow_command(client, message: Message):
    ...
    await allow_user(message.chat.id, user.id)
    await message.reply(f"✅ {user.mention} has been allowed from bio scan filter.")
